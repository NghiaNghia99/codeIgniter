...
class PosterController extends MY_Controller
{
    ...
    public function getList()
    {
        ...
        $postList = count($this->poster->getPostsByUser($this->user->id));

        if ($postList > 0) {
            $posts['results'] = $this->poster->sortPostByUserPagination($this->user->id, $limit_per_page, $start_index,
              $colName, $direction);

            $config['base_url'] = base_url('auth/content/posters');
    ...
    public function getEditPoster($id)
    {
        ...
        if ($this->checkAuthor($post['idAuthor'])) {
            ...
            if ($this->form_validation->run()) {
                ...
                $data = array(
                  ...
                  'posterTitle' => $this->input->post('title'),
                  'posterAffiliation' => $this->input->post('affiliation'),
                ...
                $this->poster->update($post['id'], $data);
                redirect('auth/content/poster/' . $post['id']);
            }
            ...
            $this->data['post'] = $post;
            $this->data['checkLinkToConference'] = $this->conferenceelement->checkExist($id, 'poster');
            $this->auth_page = 'edit_poster';
            $this->auth_content_layout();
        }
    }

    public function getRequestDoiPoster($id)
    {
        $post = $this->poster->get($id);
        if ($this->checkAuthor($post['idAuthor'])) {
            $this->session->set_userdata('post_type_doi', 'poster');
            $this->data['post'] = $post;
            $this->auth_page = 'request_doi_poster';
            $this->auth_content_layout();
        }
    }
    ...
    public function uploadPosters()
    {
        $this->session->set_userdata('active_content_sidebar', 'Poster');
        $this->data['active_content_sidebar'] = $this->session->active_content_sidebar;
        ...
        if (empty($this->session->keyword_blacklist)){
            ...
            if ($this->form_validation->run()) {
                ...
                if ($resultCheckKeyWorkBlackList){
                    ...
                    redirect(base_url('auth/content/poster/upload'));
                }
                $sharePublic = 0;
                if ($this->input->post('sharePublic') == 1){
                    $sharePublic = 1;
                ...
                $newID = $this->poster->create($data);
                if ($newID) {
                    $userID = $this->user->id;

                    $this->makeUserDir($userID);
                    $this->makeObjectDir($userID, 'posters');

                    $config['upload_path'] = FCPATH . './uploads/userfiles/' . $this->user->id . '/posters/';
                    ...
                    if ($this->upload->do_upload('file')) {
                        $dataUpdate = array(
                          'path' => '/uploads/userfiles/' . $this->user->id . '/posters/' . $newID,
                          'ext' => strtolower(pathinfo($this->upload->data('file_name'), PATHINFO_EXTENSION))
                        );

                        if ($this->poster->update($newID, $dataUpdate)){
                            $fileName = FCPATH . 'uploads/userfiles/'. $userID .'/posters/'. $this->upload->data('file_name') .'[0]';
                            $newImage = FCPATH . 'uploads/userfiles/'. $userID .'/posters/'. $newID .'.jpg';
                            $convert = $this->config->config['convert'];
                            exec("$convert $fileName $newImage");

                            $new_post = array(
                              'thumbnail' => base_url('uploads/userfiles/'. $userID .'/posters/'. $newID .'.jpg'),
                              'title' => $data['posterTitle'],
                              'date' => $data['dateOfUpload']
                            );

                            if (!empty($this->session->upload_poster)) {
                                $uploadArr = $this->session->upload_poster;
                                array_push($uploadArr, $new_post);
                            } else {
                                $uploadArr = array();
                                array_push($uploadArr, $new_post);
                            }
                            $this->session->set_userdata('upload_poster', $uploadArr);
                            ...
                            redirect('auth/content/poster/upload');
                        }
                    }
                    else{
                        $this->poster->delete($newID);
        ...
        $this->auth_page = 'upload_posters_old';
        $this->auth_content_layout();
    ...
    public function uploadEvent()
    {
        ...
        if ($newID) {
            ...
            $this->makeObjectDir($this->userID, 'posters');

            $ext = strtolower(pathinfo(basename($_FILES['file']['name']), PATHINFO_EXTENSION));
            $fileName = FCPATH . './uploads/userfiles/' . $this->userID . '/posters/' . $newID . '.' . $ext;
            if (move_uploaded_file($_FILES['file']['tmp_name'], $fileName)) {
                $dataUpdate = array(
                  'path' => '/uploads/userfiles/' . $this->userID . '/posters/' . $newID,
                  'ext' => $ext
                );

                if ($this->poster->update($newID, $dataUpdate)){
                    $fileName = $fileName .'[0]';
                    $newImage = FCPATH . 'uploads/userfiles/'. $this->userID .'/posters/'. $newID .'.jpg';
                    $convert = $this->config->config['convert'];
                    exec("$convert $fileName $newImage");

                    $new_post = array(
                      'thumbnail' => base_url('uploads/userfiles/'. $this->userID .'/posters/'. $newID .'.jpg'),
                      'title' => $data['posterTitle'],
                      'date' => $data['dateOfUpload']
                    );
