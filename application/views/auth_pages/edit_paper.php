  ...
  <div class="top-content">
    ...
    <div class="sm-menu-tab">
      <ul class="menu-post-detail">
        <a class="menu-view" href="<?= base_url('auth/content/paper/' . $post['id']) ?>">
          Paper
        </a>
        <a class="menu-edit active" href="<?= base_url('auth/content/paper/edit/' . $post['id']) ?>">
          Edit
        </a>
        <a class="menu-request-doi" href="<?= base_url('auth/content/paper/request-doi/' . $post['id']) ?>">
          Request DOI
        </a>
        <a class="menu-link-to-conference" href="<?= base_url('auth/content/paper/link-to-conference/' . $post['id']) ?>">
          Link to conference
        </a>
      </ul>
      <input type="hidden" class="get-id-post" value="<?= $post['id'] ?>">
      <button class="btn btn-popup delete-menu btn-delete-paper">
        Delete Paper
      </button>
    </div>
  </div>
    ...
    <div class="post-page-info">
      ...
      <form method="post">
        <div class="form-item">
          ...
          <input type="text" class="input input-custom" name="title" placeholder="Title"
                 value="<?php echo set_value('title', $post['paperTitle']) ?>"/>
        ...
        <div class="form-item">
          ...
          <input type="text" class="input input-custom" name="affiliation"
                 placeholder="Affiliation connected with the paper"
                 value="<?php echo set_value('affiliation', $post['paperAffiliation']) ?>">
        ...
        <div class="form-item">
          ...
          <textarea class="input textarea-custom" name="caption" placeholder="Short description of paper (3000 char max)"><?php echo set_value('caption',
        ...
        <div class="form-item">
          ...
          <input type="text" class="input input-custom" name="co_authors" placeholder="Co-authors if any"
                 value="<?php echo set_value('co_authors', $post['coAuthors']) ?>">
        ...
        <div class="form-item">
          ...
          <textarea class="input textarea-custom" name="further_reading"
                    placeholder="Publication with Additional information"><?php echo set_value('further_reading',
        ...
        <div class="form-item">
          ...
          <select id="category" name="category" class="input-custom select-custom">
        ...
        <div class="form-item">
          ...
          <select id="sub_category" name="subcategory" class="input-custom select-custom">
            <option></option>
              <?php foreach ($categories[1] as $category): ?>
                <option
                  <?php if (isset($_SESSION['get_subcategory_id']) && $subCategoryID == $category['id']) {
                      echo 'selected';
                  } ?>
                  value="<?= $category['id'] ?>"><?= $category['name'] ?></option>
              <?php endforeach; ?>
        ...
        <div class="form-item gr-button">
          ...
          <div class="d-flex">
            ...
            <button type="button" class="btn-custom btn-bg gray btn-cancel btn-redirect" data-link="<?= base_url('auth/content/paper/' . $post['id']) ?>">
              Cancel
            </button>
          </div>
        </div>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content sm-modal-content">
      ...
      <div class="sm-modal-footer d-none">
        <input type="hidden" class="get-id-post" value="<?= $post['id'] ?>">
        <button class="btn-custom btn-bg green btn-delete btn-delete-paper-confirm" data-dismiss="modal">Delete paper</button>
      </div>
    </div>
  </div>
</div>
