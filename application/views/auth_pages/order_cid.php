 ...
 */
?>
<div class="section-order-cid content-wrapper">
  <div class="title-section">
    Managed Conferences
  </div>
  <div class="block-white tab-menu-content">
    <p>
      You can use our brand new conference management toolbox for an early bird fee of only 299&euro; (approx. 325 USD)
      now. All
      you need is to fill out the following form and click the "Order now" button. The payment will be handled with
      paypal,
      but you don't have to be a paypal customer to be able to place your order.
    </p>
    <form method="post" enctype="multipart/form-data">
      <div class="form-item">
        <label class="form-label label-custom">
          ...
          <span class="unique">
            (Unique conference identifier, e.g. "myScienceCon2017")
          </span> 
        </label>
        <input type="text" class="input-custom countCharacters" name="cid" placeholder="CID"
               value="<?php echo set_value('cid') ?>"/>
      ...
      <div class="form-item">
        <label class="form-label label-custom">
          Type of Scientific Event
          <span class="req">*</span>
          
        </label>
        <select name="typeOfConference" class="input-custom select-custom-none-search form-control_SM registerInput">
<!--          <option value="">Select Type of Scientific Event</option>-->
          <?php
          $typeArr = array('Colloquium', 'Workshop', 'Conference', 'Seminar');
          foreach ($typeArr as $type) :
          ?>
          <option
            value="<?= $type ?>" <?php if ($this->session->flashdata('get_typeOfConference') && $type == $this->session->flashdata('get_typeOfConference')) {
              echo 'selected';
          } ?>><?= $type ?></option>
          <?php endforeach; ?>
        </select>
        <div class="error"><?php echo form_error('typeOfConference') ?>
        </div>
      </div>
      <div class="form-item">
        <label class="form-label label-custom">
     ...
     <!--  <div class="form-item">
        ...
        <input type="text" class="input-custom" name="paypalEmail" placeholder="Paypal email" />
        <div class="error"><?php echo form_error('paypalEmail') ?></div>
      </div> -->

      <button class="btn-custom btn-bg green btn-order">
        Order now
      </button>
    </form>
  </div>
