...
<div class="section-post-page">
  <div class="top-content">
    ...
    <div class="sm-menu-tab">
      <ul class="menu-post-detail">
        <a class="menu-view active" href="<?= base_url('auth/content/paper/' . $post['id']) ?>">
          Paper
        </a>
        <a class="menu-edit" href="<?= base_url('auth/content/paper/edit/' . $post['id']) ?>">
          Edit
        </a>
  ...
  <div class="block-white tab-menu-content view-tab-content">
    ...
    <div class="post-page-info">
      <div class="title small-title"><?= $post['paperTitle'] ?></div>
      <div class="desc-list">
        ...
        <div class="desc-list-item social-item desktop-item">
          <div style="margin-right: 3px" class="fb-like" data-href="<?= base_url('paper/' . $post['id']) ?>"
               data-width="47"
          ...
          <div class="btn-share-linked">
            ...
            <script type="IN/Share" data-url="<?= base_url('paper/' . $post['id']) ?>"></script>
          </div>
        </div>
      </div>
      <div class="content-item">
        ...
        <div class="show-info-item-mr-8 <?php if (!empty($post['paperAffiliation'])) {
            echo 'display-block';
        } ?>">
          <div>Affiliation</div>
          <div>
              <?php
              if (!empty($post['paperAffiliation'])) {
                  echo $post['paperAffiliation'];
              } else {
                  echo 'Empty';
              }
              ?>
        ...
        <div class="show-info-item-mr-8 display-block">
          ...
          <div>
              <?php
              echo '<a class="link" href="' . $this->config->base_url() . 'paper/' . $post['id'] . '">' . $this->config->base_url() . 'paper/' . $post['id'] . '</a>';
              ?>
          </div>
        </div>
        <div class="download-item">Do you have problems viewing the pdf-file? Download paper
            <a class="link" href="<?= base_url($post['path'] . '.pdf') ?>" download="paper.pdf">here</a>
        </div>
        <div class="qr-item desktop-item">
          <img
            src="http://chart.apis.google.com/chart?chs=90x90&cht=qr&chl=<?= base_url('paper/' . $post['id']) ?>">
        </div>
      </div>
