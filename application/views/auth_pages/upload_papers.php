  ...
  <form id="uploadContentForm" method="post" enctype="multipart/form-data">
    ...
    <div class="form-item">
      ...
      <textarea type="text" class="input-custom textarea-custom countCharacters" name="caption"
                placeholder="Short description of paper (1000 chars max)"><?php echo set_value('caption') ?></textarea>
      <div class="gr-text-after">
        ...
        <div class="text-right">
          <input type="hidden" id="caption" class="limit-character" value="1000">
    ...
    <div id="video_upload_btn" class="button-bottom d-none">
      <input type="button" id="submitUploadContentForm" data-type="paper" class="btn-custom btn-bg green btn-upload ml-auto" value="Upload" name="submit">
    </div>
      <?php if ($this->session->flashdata('upload_post_msg')) : ?>
        <div class="button-bottom btn-reset">
          <a href="<?= base_url('auth/content/paper/upload') ?>" class="add-spinner btn-custom no-child btn-bg green btn-upload ml-auto">Reset</a>
        </div>
      <?php endif; ?>
    <div class="sm-text-item">
      <div class="title-sm-text-item">
        Uploaded papers this session
      </div>
      <div class="content-sm-text-item">
        The following list contains thumbnail images of your uploaded papers in this session.
      </div>
    </div>
    <div class="sm-table-item">
      <div class="title-sm-table-item">
        Papers in your job list
      </div>
      <table class="table table-custom table-preview-upload">
        ...
        <tbody>
        <?php
        $list_post = $this->session->upload_paper;
        if (!empty($list_post)):
...
<?php else: ?>
  <div class="block-white-p40-100">
    <p>You had failed to upload this paper due to our terms and conditions. Please upload a new paper or contact our team for more information.</p>
    <a class="link mt-2" href="<?= base_url('auth/content/paper/upload') ?>">Upload a new paper here.</a>
  </div>
<?php endif; ?>